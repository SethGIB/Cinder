...
print(f"Inserting {sys.argv[3]} before any line containing {sys.argv[2]}")
	...
	for name in files:
		...
		for line in fileLines:
			if line.find( sys.argv[3] ) != -1:
				alreadyFound = True
				print(f"Already found in {str(join(root,name))}")
		if not alreadyFound:
			f = open( join(root, name), "w" )
			f.seek( 0, os.SEEK_SET )
			for line in fileLines:
				if line.find( sys.argv[2] ) != -1:
					print(f"Inserting before line {str(join(root,name))}")
					print(line)
